...
async function createSpecBundle(runId: string, projectId: string, sourceBranch: string, repo: string, planText: string): Promise<{ specBundleId: string; manifestPath: string }> {
  ...
  const files: Record<string, string> = {
    ...
    "tasks.json": JSON.stringify({ tasks: [{ id: "task-1", title: "Implement planned changes" }] }, null, 2),
  ...
  for (const [name, content] of Object.entries(files)) {
    await putObject(`${prefix}/${name}`, content, name.endsWith(".json") ? "application/json" : "text/markdown");
  }
