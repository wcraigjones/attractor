...
function lintReachability(graph: DotGraph, diagnostics: DotDiagnostic[]): void {
  ...
  const exits = new Set(exitNodeIds(graph));
  for (const nodeId of visited) {
    if (exits.has(nodeId)) {
      continue;
    }
    const outgoingCount = graph.edges.filter((edge) => edge.from === nodeId).length;
    if (outgoingCount === 0) {
      diagnostics.push({
        rule: "deadend",
        severity: "ERROR",
        message: `Node ${nodeId} is a deadend (no outgoing edges)`,
        nodeId
      });
    }
  }
  ...
  while (reverseQueue.length > 0) {
    ...
    for (const edge of graph.edges) {
      if (edge.to === nodeId && !reverseVisited.has(edge.from)) {
  ...
  for (const nodeId of visited) {
    if (!reverseVisited.has(nodeId)) {
      diagnostics.push({
        rule: "reachability",
        severity: "ERROR",
        message: `Node ${nodeId} has no path to exit`,
        nodeId
      });
    }
  }
