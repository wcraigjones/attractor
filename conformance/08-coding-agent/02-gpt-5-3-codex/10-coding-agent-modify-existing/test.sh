...
sed -e "s|__MODEL__|gpt-5.3-codex|g" -e "s|__WORKDIR__|$WORKDIR|g" "$TEST_DIR/pipeline.dot" > "$RUNTIME_PIPELINE"
...
if [[ "$PIPELINE_EXIT" -ne 0 ]]; then
    if echo "$PIPELINE_STDERR" | grep -qiE "does not exist|not supported|not a chat model|model_not_found|not found|INTERNAL|Internal error|RESOURCE_EXHAUSTED|rate limit|quota|overloaded|500|503"; then
        skip "gpt-5.3-codex: model not available"
    fi
fi
...
pass "gpt-5.3-codex: coding agent modify-existing scenario passed"
